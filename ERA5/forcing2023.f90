PROGRAM forcing2023
    ...
    integer :: onx,ony,realnt,halfnt
    integer :: ncids(8),varids(9)
    ...
    character(len=4) :: varname(9)
    character(len=10) :: buffer,names(8),vnam(9),tdim(8)
    character(len=30) :: filename,outfile,units(9)
    character(len=100) :: grid, longnames(9),tnames(8)
    logical :: yleap,ex1
...
102 FORMAT('baltic_',a,'_',i4.4,'.nc')
    ...
    longnames(7) = "shortwave downward radiation"
...
!END READ GRID
    ...
    allocate( lat_tmp(ny), input_tmp(nx,ny,nt), time(realnt) )
    ...
    do i = 1,9
      ...
      CALL check(nf90_get_var(ncid,varid,input_tmp(:,:,1:halfnt-1), &
        start=(/1,1,1,2/),count=(/nx,ny,1,halfnt-1/) ),30+i)
      CALL check(nf90_get_var(ncid,varid,input_tmp(:,:,halfnt:nt-1), &
        start=(/1,1,2,halfnt+1/),count=(/nx,ny,1,nt-halfnt/) ),30+i)
...
!$OMP& SHARED(lon,lat,olon,olat,ncids,varids), SCHEDULE(DYNAMIC)
    do i = 1, nt
      ...
      CALL interp(nx,ny,onx,ony,lon,lat,olon,olat,input(:,:,i,6),output(:,:,10))  
      ...
      CALL interp(nx,ny,onx,ony,lon,lat,olon,olat,input(:,:,i,9),output(:,:,9))
      CALL spec_humid(onx,ony,output(:,:,10),output(:,:,5),output(:,:,6))
      ...
      !SWRAD_DOWN
      where(output(:,:,7).lt.0) output(:,:,7) = 0.0
      output(:,:,7) = output(:,:,7)/3600
      ...
      do j = 1, 8
        CALL check( nf90_put_var( ncids(j), varids(j), real(output(:,:,j),4),&
             start = (/ 1, 1, i /) ), 518 )
        if (j.eq.8) then
          CALL check( nf90_put_var( ncids(j), varids(9), real(output(:,:,9),4),&
             start = (/ 1, 1, i /) ), 519 )
        end if
      end do
      !$OMP END CRITICAL
