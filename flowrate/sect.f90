PROGRAM sect
    ...
    integer :: i,j,k,v_j,v_istr,v_iend,vid(4),n,cnt
    ...
    real(kind=8), allocatable :: zdif(:), frate(:), salt(:,:,:), temp(:,:,:)
    character(len=200) :: filename,path
    character(len=50) :: tunits,tcal

    100 format(a,'/ocean_avg_',i4,'-',i2.2,'-',i2.2,'.nc')
    path = '/users/work/mmuzyka/CSDIR/metro_560x600_era2004/run/baltic'
...
!READ GRID
    ...
    allocate( salt(nx,ny,nz), temp(nx,ny,nz) )
...
!END READ GRID
    ...
    CALL check( nf90_create( 'flowrate_2004_test.nc',NF90_NETCDF4,ncid ), 500 )
    ...
    CALL check(nf90_def_var( ncid, 'salt', NF90_DOUBLE,&
        (/tdimid/), vid(4) ), 508)
    CALL check( nf90_put_att( ncid, vid(4), 'longname',&
        'salt flow rate' ), 305 )
    CALL check( nf90_put_att( ncid, vid(4), 'units',&
        'kg/s' ), 305 )
    ...
    do
        ...
        CALL check(nf90_inq_varid(nc,"salt",varid),319)
        CALL check(nf90_get_var(nc,varid,salt,start=(/1,1,1,n/)),320)

        CALL check(nf90_inq_varid(nc,"temp",varid),319)
        CALL check(nf90_get_var(nc,varid,temp,start=(/1,1,1,n/)),320)
    ...
    myfx = 0.0
    do i = v_istr, v_iend
      write(*,*) salt(i,v_j,nz),salt(i,v_j+1,nz)
...
!    CALL check( nf90_close(ncid), 600 )
    ...
    deallocate( zdif, frate, salt, temp )
END PROGRAM sect
