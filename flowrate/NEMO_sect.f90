PROGRAM NEMO_SECT
    ...
    integer :: ncid, varid, i, k, nc, tdimid, vid(2), cnt
    integer :: date(3), datestp(3), jstr(1), istr(1), iend(1)
    real(kind=8) :: lon_tr(xin), lat_tr(yin), depth_tr(zin), hz_tr(zin)
    real(kind=8) :: hlat,hlon(2),fv,d,dx(xin),flux,time 
    ...
    CALL check( nf90_create( 'flowrate_NEMO.nc',NF90_NETCDF4,nc ), 500 )
    ...
    CALL check(nf90_def_var( nc, 'flowrate', NF90_DOUBLE,&
        (/tdimid/), vid(2) ), 508)
...
!READ TRACERS FROM FILES
    ...
    do
      ...
      if (first) then  
        ...
        istr = minloc(abs(lon_tr-hlon(1)))
        iend = minloc(abs(lon_tr-hlon(2)))
        jstr = minloc(abs(lat_tr-hlat))
...
!      CALL check(nf90_get_var(ncid,vid,uvelin),18)
      ...
      if (first) then
        do i = istr(1),iend(1)
          CALL haversine((/lon_tr(i-1),lon_tr(i)/),&
              (/lat_tr(jstr(1)),lat_tr(jstr(1))/),d)
          dx(i) = d*0.5
          CALL haversine((/lon_tr(i),lon_tr(i+1)/),&
              (/lat_tr(jstr(1)),lat_tr(jstr(1))/),d)
          dx(i) = dx(i)+d*0.5
          write(*,*) 'i=',i,'dx=',dx(i),'m'
        end do
      ...
      do i = istr(1),iend(1)
      do k = 1, zin
        if (vvelin(i,jstr(1),k).eq.fv) EXIT
        flux = flux+vvelin(i,jstr(1),k)*dx(i)*hz_tr(k)
      end do
      end do

      write(*,*) 'flux=',flux*1e-6
      CALL check( nf90_put_var( nc, vid(1), time, start=(/cnt/)), 517 )
      CALL check( nf90_put_var( nc, vid(2), flux*1e-6, start=(/cnt/)), 517 )
      cnt = cnt+1
    ...
    deallocate( tempin, saltin, uvelin, vvelin ) 
!END READ TRACERS

END PROGRAM NEMO_SECT
