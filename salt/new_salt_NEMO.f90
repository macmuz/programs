...
!    date1 = (/2023,9,10/)
    ...
    allocate( lon(nx), lat(ny), depth(nz), dz(nz), f(nx,ny), heat(nx,ny,nz) )
    allocate( mask(nx,ny,nz),tmp2d(nx,ny),v(nx,ny,nz),mass(nx,ny,nz) )
    allocate( temp(nx,ny,nz),salt(nx,ny,nz),den(nx,ny,nz),mask2d(nx,ny) )
    ...
    CALL check(nf90_get_var(ncid,varid,temp,start=(/1,1,1,1/),&
        count=(/nx,ny,nz,1/)),28)
...
!    write(*,*) nx,ny,nz
    ...
    where(mask.eq.1 .and. temp.lt.-900) mask = 0
...
!    end do
    ...
    cnt = 0
    CALL check(nf90_create( "NEMO_salt0.nc", NF90_NETCDF4, oncid ), 200)
...
!    CALL check( nf90_put_var( ncid, vid(7), d(1,1,:) ), 212 )
    ...
    deallocate( mask, tmp2d, v, mass ) 
    deallocate( temp,salt,den, heat, mask2d, tmp4d )

END PROGRAM new_salt_NEMO
