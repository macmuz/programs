PROGRAM salt_ROMS
    ...
    real(kind=8), allocatable :: lon(:,:),lat(:,:),f(:,:),v(:,:,:),mass(:,:,:)
    real(kind=8), allocatable :: Cs_w(:),s_w(:),h(:,:),depth(:,:,:)
    real(kind=8), allocatable :: Cs_r(:),s_r(:),zeta(:,:),heat(:,:,:)
    real(kind=8), allocatable :: temp(:,:,:),salt(:,:,:),den(:,:,:)
    real(kind=8) :: time,hc,tref,Cp
...
!    date1 = (/1999,1,6/)
    ...
    allocate( lon(nx-1,ny-1), lat(nx-1,ny-1), depth(nx,ny,nz), f(nx,ny) )
    allocate( Cs_w(nz+1), s_w(nz+1), Cs_r(nz), s_r(nz), h(nx,ny) )
    allocate( mask(nx,ny),v(nx,ny,nz),mass(nx,ny,nz),zeta(nx,ny) )
    allocate( temp(nx,ny,nz),salt(nx,ny,nz),den(nx,ny,nz),heat(nx,ny,nz) ) 
    ...
    cnt = 0
    CALL check(nf90_create( "ROMS_salt_uerraGLSold_new.nc", NF90_NETCDF4, oncid ), 200)
    ...
    do
        write(*,*) trim(path)
        CALL check(nf90_open(trim(path),NF90_NOWRITE,ncid),10)
        CALL check(nf90_inq_varid(ncid,"temp",myvarid(1)),127)
        CALL check(nf90_inq_varid(ncid,"salt",myvarid(2)),129)
        CALL check(nf90_inq_varid(ncid,"zeta",myvarid(3)),131)
        CALL check(nf90_inq_varid(ncid,"ocean_time",myvarid(4)),133)
        do t = 1,nt
          CALL check(nf90_get_var(ncid,myvarid(1),temp,start=(/1,1,1,t/)),128)
          CALL check(nf90_get_var(ncid,myvarid(2),salt,start=(/1,1,1,t/)),130)
          CALL check(nf90_get_var(ncid,myvarid(3),zeta,start=(/1,1,t/)),132)
          CALL check(nf90_get_var(ncid,myvarid(4),time,start=(/t/)),134)

          do k = 1,nz
            where(mask.eq.0) temp(:,:,k)=0.0
            where(mask.eq.0) salt(:,:,k)=0.0
          end do
...
!    CALL check( nf90_put_var( ncid, vid(7), d(1,1,:) ), 212 )
    ...
    deallocate( lon, lat, depth, f )
    deallocate( h, Cs_w, s_w, Cs_r, s_r )
    deallocate( mask, v, mass, zeta ) 
    deallocate( temp,salt,den,heat )

END PROGRAM salt_ROMS
