PROGRAM ROMS_BC
    ...
    integer :: nx, ny, nz, i, j, k, l, m, x, y
    ...
    logical :: mask_tr(xin,yin,zin),ex,mask2d(xin,yin),new_mask_tr(xin,yin,zin)
...
!    nt_tr = 20 !!!!!!TESTOWE!!!!!!!!!!!!!!!
    ...
    allocate( uvelin(xin,yin,zin,nt_tr+2), vvelin(xin,yin,zin,nt_tr+2) ) 
    allocate( ubarin(xin,yin,nt_tr+2), vbarin(xin,yin,nt_tr+2) ) 
!END FIND
...
!FIND NORTH BC
    ...
    allocate( ubar(nx-1,24*nt_tr), vbar(nx,24*nt_tr) )
!    allocate( ubar(nx-1,nt_tr), vbar(nx,nt_tr) )
    allocate( salt(nx,nz,24*nt_tr), time(nt_zeta), timed(nt_tr) )
    ...
    allocate( ubar_day(span-1,nt_tr+2), vbar_day(span,nt_tr+2))
...
!        end do
      ...
      do k = 1,56
        vvelin(:,:,k,i) = tmp_tr(:,:,57-k)
      end do


      if (first) then
        mask_tr = .true. 
        where( abs(tempin(:,:,:,i)-fv).lt.1.0 ) mask_tr = .false.   
        mask2d = .true. 
        where( abs(uvelin(:,:,zin,i)-fv).lt.1.0 ) mask2d = .false.   
      ...
      CALL extrap3d(tempin(:,:,:,i),new_mask_tr,xin,yin,zin,500,2)
      CALL extrap3d(saltin(:,:,:,i),new_mask_tr,xin,yin,zin,500,2)
      ...
      do l = 1,yin
        if (mask2d(k,l).eq..true.) then
          do m = 1,zin
            ...
            ubarin(k,l,i) = ubarin(k,l,i)+hz_tr(m)*uvelin(k,l,m,i)
            vbarin(k,l,i) = vbarin(k,l,i)+hz_tr(m)*vvelin(k,l,m,i)
          end do
          ubarin(k,l,i) = ubarin(k,l,i)/sum(hz_tr,mask=mask_tr(k,l,:))
          vbarin(k,l,i) = vbarin(k,l,i)/sum(hz_tr,mask=mask_tr(k,l,:))
        end if
      end do
      end do
      CALL extrap(ubarin(:,:,i),mask_tr(:,:,zin),xin,yin,500,0)
      CALL extrap(vbarin(:,:,i),mask_tr(:,:,zin),xin,yin,500,0)
...
!END FILL
    ...
    CALL check( nf90_def_dim( ncid, 'xi_u', nx-1, xi_u_d ), 301 )
    CALL check( nf90_def_dim( ncid, 'xi_v', nx, xi_v_d ), 301 )
    ...
    CALL check( nf90_def_dim( ncid, 'v2d_time', 24*nt_tr, v2d_t_d ), 404 )
!    CALL check( nf90_def_dim( ncid, 'v2d_time', nt_tr, v2d_t_d ), 404 )
    ...
    CALL check(nf90_def_var( ncid, 'v2d_time', NF90_DOUBLE,&
        (/v2d_t_d/),varid(9) ), 312)
    CALL check( nf90_put_att( ncid, varid(9), 'long_name',&
        '2D momentum time' ), 313 )
    CALL check( nf90_put_att( ncid, varid(9), 'units',&
        'days since 1968-05-23 00:00:00 GMT' ), 314 )
    CALL check( nf90_put_att( ncid, varid(9), 'calendar',&
        'gregorian' ), 315 )

    CALL check(nf90_def_var( ncid, 'lon_u', NF90_DOUBLE, (/xi_u_d/),&
               varid(10) ), 327)
    ...
    CALL check(nf90_def_var( ncid, 'lon_v', NF90_DOUBLE, (/xi_v_d/),&
               varid(11) ), 327)
    ...
    CALL check(nf90_def_var( ncid, 'ubar_north', NF90_FLOAT, dimids2ubar,&
               varid(12) ), 320)
    ...
    CALL check(nf90_def_var( ncid, 'vbar_north', NF90_FLOAT, dimids2vbar,&
               varid(13) ), 320)
    CALL check( nf90_put_att( ncid, varid(13), 'long_name',&
        '2D v-momentum northern boundary condition' ), 321 )
    CALL check( nf90_put_att( ncid, varid(13), 'units',&
        'meter second-1' ), 322 )
    CALL check( nf90_put_att( ncid, varid(13), 'time',&
        'v2d_time' ), 323 )
    ...
    CALL check( nf90_put_var( ncid, varid(9), time ), 345 )
    CALL check( nf90_put_var( ncid, varid(10), lonu(:,jbc) ), 347 )
    CALL check( nf90_put_var( ncid, varid(11), lonv(:,jbc-1) ), 347 )
    CALL check( nf90_put_var( ncid, varid(12), real(ubar,4) ), 346 )
    CALL check( nf90_put_var( ncid, varid(13), real(vbar,4) ), 346 )
    CALL check(nf90_close(ncid),350)

    deallocate( skag_zeta, dis_array, gote_zeta, tr_tmp )
    deallocate( Cs_r, s_rho, h, lon, lat, mask)
    deallocate( idx, W, zeta, temp, salt, y2a, timed )
    ...
    deallocate( lonu, latu, lonv, latv, ubar_day, vbar_day )
END PROGRAM ROMS_BC
