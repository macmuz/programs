PROGRAM new_ROMS_BC
    ...
    real(kind=8) :: tmpzin(zin), tmp_tr(xin,yin,zin,12)  
...
!READ command line arg: YEAR
    ...
    id = index(buffer,'.nc')
    write(fileout,'(A,A,i4.4,A)') trim(buffer(1:id-1)),'_bc_bar_',year,'.nc'
!READ command line
...
!END CREATE
    ...
    nt_tr = 24*365
!    nt_tr = 24
    if (leapy) nt_tr = 24*366
!    nt_tr = 20 !!!!!!TESTOWE!!!!!!!!!!!!!!!
    allocate( dis_array(xin,yin),y2a(xin,yin) )
    allocate( tempin(xin,yin,zin,12), saltin(xin,yin,zin,12) ) 
    allocate( uvelin(xin,yin,zin,12), vvelin(xin,yin,zin,12) ) 
    allocate( ubarin(xin,yin,12), vbarin(xin,yin,12), zetain(xin,yin,12) ) 
!END FIND
...
!FIND NORTH BC
    ...
    allocate( zeta(nx,12), temp(nx,nz,12) )
    allocate( ubar(nx-1,12), vbar(nx,12) )
!    allocate( ubar(nx-1,nt_tr), vbar(nx,nt_tr) )
    allocate( salt(nx,nz,12), time(12) )
...
!READ TRACERS FROM FILES
    100 format('new_files/'i4.4,'/',i2.2,'/BAL-NEMO_PHY-',i4,i2.2,i2.2,i2.2,'.nc')
    date = (/year-1,12,31/)
    write(prevfile,100) date(1),date(2),date(1),date(2),date(3),12
...
!    end do
    ...
    do ho = 0,12,12
      ...
      write(trfile,100) date(1),date(2),date(1),date(2),date(3),ho
      write(*,*) trim(trfile)
      ...
      if (first) then
        ...
        INQUIRE(FILE=trim(prevfile),EXIST=ex)
        if (.not.ex) then
          prevfile = trfile
          id = 1 
        end if
      ...
      CALL check(nf90_open(trim(prevfile),NF90_NOWRITE,ncid2),1016)

      CALL check(nf90_inq_varid(ncid2,"thetao",vid2),117)
      CALL check(nf90_get_var(ncid2,vid2,tmp_tr(:,:,:,1),start=(/30,250,1,id/),&
          count=(/xin,yin,zin,1/)),18)

      CALL check(nf90_inq_varid(ncid,"thetao",vid),117)
      CALL check(nf90_get_var(ncid,vid,tmp_tr(:,:,:,2:12),start=(/30,250,1,1/),&
          count=(/xin,yin,zin,11/)),18)
      ...
      CALL check(nf90_inq_varid(ncid2,"so",vid2),117)
      CALL check(nf90_get_var(ncid2,vid2,tmp_tr(:,:,:,1),start=(/30,250,1,id/),&
          count=(/xin,yin,zin,1/)),18)
      ...
      CALL check(nf90_inq_varid(ncid2,"uo",vid2),117)
      CALL check(nf90_get_var(ncid2,vid2,tmp_tr(:,:,:,1),start=(/30,250,1,id/),&
          count=(/xin,yin,zin,1/)),18)
      ...
      CALL check(nf90_inq_varid(ncid2,"vo",vid2),117)
      CALL check(nf90_get_var(ncid2,vid2,tmp_tr(:,:,:,1),start=(/30,250,1,id/),&
          count=(/xin,yin,zin,1/)),18)
      ...
      CALL check(nf90_inq_varid(ncid2,"sla",vid2),117)
      CALL check(nf90_get_var(ncid2,vid2,zetain(:,:,1),start=(/30,250,id/),&
          count=(/xin,yin,1/)),18)

      CALL check(nf90_inq_varid(ncid,"sla",vid),417)
      CALL check(nf90_get_var(ncid,vid,zetain(:,:,2:12),start=(/30,250,1/),&
          count=(/xin,yin,11/)),18)
!      write(*,*) minval(zetain),maxval(zetain)

      CALL check(nf90_close(ncid2),23)

      if (first) then
        mask_tr = .true. 
        where( abs(tempin(:,:,:,1)-fv).lt.1.0 ) mask_tr = .false.   
        mask2d = .true. 
        where( abs(zetain(:,:,1)-fv).lt.1.0 ) mask2d = .false.   
      ...
      do t = 1,12
        CALL extrap3d(tempin(:,:,:,t),new_mask_tr,xin,yin,zin,300,2)
        CALL extrap3d(saltin(:,:,:,t),new_mask_tr,xin,yin,zin,300,2)
        ...
        do l = 1,yin
          if (mask2d(k,l).eq..true.) then
            do m = 1,zin
              ...
              ubarin(k,l,t) = ubarin(k,l,t)+hz_tr(m)*uvelin(k,l,m,t)
              vbarin(k,l,t) = vbarin(k,l,t)+hz_tr(m)*vvelin(k,l,m,t)
            end do
            ubarin(k,l,t) = ubarin(k,l,t)/sum(hz_tr,mask=mask_tr(k,l,:))
            vbarin(k,l,t) = vbarin(k,l,t)/sum(hz_tr,mask=mask_tr(k,l,:))
          end if
        end do
        end do
        CALL extrap(ubarin(:,:,t),mask_tr(:,:,zin),xin,yin,300,0)
        CALL extrap(vbarin(:,:,t),mask_tr(:,:,zin),xin,yin,300,0)
        CALL extrap(zetain(:,:,t),mask_tr(:,:,zin),xin,yin,300,0)

        call spline3d(xin,yin,zin,lon_tr,lat_tr,depth_tr,&
        span,nz,lon(istr:iend,jbc),lat(istr:iend,jbc),&
        depth(istr:iend,:),tempin(:,:,:,t),temp_day(:,:))

        call spline3d(xin,yin,zin,lon_tr,lat_tr,depth_tr,&
        span,nz,lon(istr:iend,jbc),lat(istr:iend,jbc),&
        depth(istr:iend,:),saltin(:,:,:,t),salt_day(:,:))

        CALL splie2(lat_tr,xin,yin,ubarin(:,:,t),y2a)
        do x = ibcstr, ibcend-1
          do i = 1, xin
            lon_val(i) = splint(lat_tr,ubarin(i,:,t),y2a(i,:),yin,latu(x,jbc))
          end do
          call spline(lon_tr,lon_val,xin,y2_1d)
          ubar_day(x-ibcstr+1) = splint(lon_tr,lon_val,y2_1d,xin,lonu(x,jbc))
        end do

        CALL splie2(lat_tr,xin,yin,vbarin(:,:,t),y2a)
        do x = ibcstr, ibcend
          do i = 1, xin
            lon_val(i) = splint(lat_tr,vbarin(i,:,t),y2a(i,:),yin,latv(x,jbc-1))
          end do
          call spline(lon_tr,lon_val,xin,y2_1d)
          vbar_day(x-ibcstr+1) = splint(lon_tr,lon_val,y2_1d,xin,lonv(x,jbc-1))
        end do

        CALL splie2(lat_tr,xin,yin,zetain(:,:,t),y2a)
        do x = ibcstr, ibcend
          do i = 1, xin
            lon_val(i) = splint(lat_tr,zetain(i,:,t),y2a(i,:),yin,lat(x,jbc))
          end do
          call spline(lon_tr,lon_val,xin,y2_1d)
          zeta_day(x-ibcstr+1) = splint(lon_tr,lon_val,y2_1d,xin,lon(x,jbc))
        end do

        temp(istr:iend,:,t) = temp_day
        salt(istr:iend,:,t) = salt_day
        ubar(istr:iend-1,t) = ubar_day
        vbar(istr:iend,t) = vbar_day
        zeta(istr:iend,t) = zeta_day
        do k = 1,nz
          temp(1:istr-1,k,t)=temp(istr,k,t)
          salt(1:istr-1,k,t)=salt(istr,k,t)
          temp(iend+1:nx,k,t)=temp(iend,k,t)
          salt(iend+1:nx,k,t)=salt(iend,k,t)
        end do
        zeta(1:istr-1,t)=zeta(istr,t)
        zeta(iend+1:nx,t)=zeta(iend,t)
      end do

1000    continue                

        CALL elapsed(date,date_0,julian)
        time(1) = real(julian,8)+real(ho,8)/24.0
        do j = 2,12
          time(j) = time(j-1)+real(1.0/24.0,8)
        end do

        CALL check( nf90_put_var( oncid, varid(1), time, start=(/(cnt-1)*12+1/) ), 341 )
        CALL check( nf90_put_var( oncid, varid(2), real(temp,4), &
           start=(/1,1,(cnt-1)*12+1/) ), 342 )
        CALL check( nf90_put_var( oncid, varid(3), time, start=(/(cnt-1)*12+1/) ), 343 )
        CALL check( nf90_put_var( oncid, varid(4), real(zeta,4), &
           start=(/1,(cnt-1)*12+1/) ), 344 )
        CALL check( nf90_put_var( oncid, varid(5), time, start=(/(cnt-1)*12+1/) ), 345 )
        CALL check( nf90_put_var( oncid, varid(6), real(salt,4), &
           start=(/1,1,(cnt-1)*12+1/) ), 346 )
        CALL check( nf90_put_var( oncid, varid(9), time, start=(/(cnt-1)*12+1/) ), 345 )
        CALL check( nf90_put_var( oncid, varid(12), real(ubar,4), &
           start=(/1,(cnt-1)*12+1/) ), 346 )
