PROGRAM getNEMO
    ...
    character(len=200) :: sstfile
    character(len=30) :: coord
    real(kind=8) :: W(2,2),pt(2),depth
    ...
    CALL check(nf90_open(trim(sstfile),NF90_NOWRITE,ncid),310)
    ...
    CALL check(nf90_inq_varid(ncid,'depth',vid),121)
    CALL check(nf90_get_var(ncid,vid,depth),18)
    ...
    CALL check(nf90_create('thetao_pt.nc',NF90_NETCDF4,ncid2),310)
    ...
    CALL check(nf90_inq_varid(ncid,'thetao',vid),124)
    CALL check(nf90_get_var(ncid,vid,data3d,&
        start=(/1,1,1,1/),count=(/ni,nj,1,nt/)),18)
