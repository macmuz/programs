PROGRAM energy_NEMO
    ...
    integer :: xd,yd,zd,td,cnt
    integer :: oncid,i,j,k
    real, allocatable :: depth(:)
    real(kind=8), allocatable :: lon(:),lat(:),f(:,:),vol(:,:,:)
    real(kind=8), allocatable :: u(:,:,:),v(:,:,:),dz(:),tmp3d(:,:,:)
    real(kind=8) :: time,tref,Cp,tote,totvol,tmp
...
!    date1 = (/1999,1,6/)
    ...
    allocate( mymask(nx,ny,nz),tmp3d(nx,ny,nz),vol(nx,ny,nz) )
    allocate( u(nx,ny,nz),v(nx,ny,nz),mask2d(nx,ny) ) 
    ...
    CALL check(nf90_get_var(ncid,varid,u),28)
...
!    write(*,*) nx,ny,nz
    ...
    do 
      ...
      do i = 2,nx-1
        do j = 2,ny-1
          if (mymask(i,j,1)) then
            if ( .not.mymask(i-1,j,1) .and. mask2d(i-1,j).eq.1 ) then
              mymask(i-1,j,:) = 1
              cnt = cnt+1
            end if
            if ( .not.mymask(i+1,j,1) .and. mask2d(i+1,j).eq.1 ) then
            ...
            if ( .not.mymask(i,j-1,1) .and. mask2d(i,j-1).eq.1 ) then
              mymask(i,j-1,:) = 1
              cnt = cnt+1
            end if
            if ( .not.mymask(i,j+1,1) .and. mask2d(i,j+1).eq.1 ) then
    ...
    where(mymask .and. u.lt.-900.0) mymask = .false.

    dz(1) = depth(1)*2
    do i = 2,nz
      dz(i) = (depth(i)-sum(dz(1:i-1)))*2 
    end do
    CALL field(nx,ny,lon,lat,f)
!    write(*,*) f(1,ny)
    do i = 1,nx
      do j = 1,ny
        do k = 1,nz
          vol(i,j,k) = f(i,j)*dz(k)
        end do
      end do
    end do
    totvol = sum(vol,mask=mymask)
!    write(*,*) totvol,sum(vol)

!    tmp = 0.0
!    do i = 1,nx
!    do j = 1,ny
!    do k = 1,nz
!      if (mymask(i,j,k)) then
!        tmp = tmp+vol(i,j,k)
!      end if 
!    end do
!    end do
...
!    write(*,*) tmp
    ...
    mask2d = 0
    where(mymask(:,:,1)) mask2d = 1
!    do i = 1,nz
!        write(*,*) i,dz(i)
!    end do

    cnt = 0
    CALL check(nf90_create( "NEMO_energy.nc", NF90_NETCDF4, oncid ), 200)
    ...
    CALL check(nf90_put_att( oncid, vid(1), 'calendar', 'gregorian'),220)
    ...
    do
        ...
        CALL check(nf90_get_var(ncid,varid,v),28)
        CALL check(nf90_inq_varid(ncid,"time",varid),27)
        CALL check(nf90_get_var(ncid,varid,time),28)
        CALL check(nf90_close(ncid),35)

        where(.not.mymask) u=0
        where(.not.mymask) v=0
!        CALL dens(nx,ny,nz,temp,salt,depth,den)
        tote = sum(0.5*(u**2+v**2)*vol,mask=mymask)
!        tmp3d = u**2+v**2
!        write(*,*) tote/totvol,tmp3d(360,200,1),u(360,200,1),v(360,200,1),vol(360,200,1)
 
        cnt = cnt+1
        CALL check( nf90_put_var( oncid, vid(1), time, start = (/cnt/) ), 209 )
        CALL check( nf90_put_var( oncid, vid(2), tote/totvol, start = (/cnt/) ), 209 )
...
!    CALL check(nf90_def_dim( ncid, 'z', nz, zd ), 203)
    ...
    CALL check(nf90_def_var( ncid, 'mask', NF90_INT, (/xd,yd/), vid(2)), 204) 
...
!    where(.not.mask(:,:,1)) f=0.0
    CALL check( nf90_put_var( ncid, vid(1), f*1e-6 ), 211 )
    CALL check( nf90_put_var( ncid, vid(2), mask2d ), 211 )
...
!    CALL check( nf90_put_var( ncid, vid(7), d(1,1,:) ), 212 )
    ...
    deallocate( lon, lat, depth, dz, f )
    deallocate( mymask, tmp3d, vol ) 
    deallocate( u, v, mask2d )

END PROGRAM energy_NEMO
...
SUBROUTINE dens(nx,ny,nz,temp,salt,depth,den)
    ...
    real(kind=8), intent(in) :: temp(nx,ny,nz),salt(nx,ny,nz),depth(nz)
...
SUBROUTINE field(nx,ny,lon,lat,f)
    implicit none
