PROGRAM new_ROMS_ini
    ...
    real(kind=8) :: hc,point(2),corners(4,2),time(1),line,tmp_rho,fv,zw(2)
    ...
    character(len=250) :: grid,outgrid,infile,path
    character(len=20) :: buffer,file1(3),file2(3),subpath
...
!READ INPUT FILENAME AND DATE
    ...
    idx = index(grid,'.nc')
    write(outgrid,"(A,'_',i4.4,'_',i2.2,'_',i2.2,A)") &
        trim(grid(1:idx-1)),YEAR,MON,DAY,'_initial_NEMO.nc'
    
    CALL sub_day(date,.true.)
    write(*,*) date 

    write(subpath,"('/',i4.4,'/',i2.2,'/')") date(1),date(2) 
    write(infile,'(A,i4.4,i2.2,i2.2,A)') &
        'BAL-NEMO_PHY-',date(1),date(2),date(3),'12.nc'
...
!END CALCULATIONS OF SIGMA COORD VARIABLES
    ...
    CALL check(nf90_open(trim(path)//trim(subpath)//trim(infile),&
        NF90_NOWRITE,ncid),40)
    ...
    CALL check(nf90_inq_varid(ncid,"sla",vid),47)
    CALL check(nf90_get_var(ncid,vid,zetain,&
        start=(/1,1,12/),count=(/xin,yin,1/)),46)
    ...
    CALL check(nf90_get_var(ncid,vid,tmpxyzin,&
        start=(/1,1,1,12/),count=(/xin,yin,zin,1/)),48)    
    do k = 1,zin
      tempin(:,:,k) = tmpxyzin(:,:,1+zin-k)
    end do
    CALL check(nf90_inq_varid(ncid,"so",vid),49)
    CALL check(nf90_get_var(ncid,vid,tmpxyzin,&
        start=(/1,1,1,12/),count=(/xin,yin,zin,1/)),50)    
    do k = 1,zin
      saltin(:,:,k) = tmpxyzin(:,:,1+zin-k)
    end do

    CALL check(nf90_inq_varid(ncid,"uo",vid),47)
    CALL check(nf90_get_var(ncid,vid,tmpxyzin,&
        start=(/1,1,1,12/),count=(/xin,yin,zin,1/)),48)
    do k = 1,zin
...
!___U___
    ...
    if (ex1 .and. ex2) then
      ...
      idx_u = 0
      W_u = 0
!$OMP PARALLEL DO DEFAULT(PRIVATE),SHARED(lonu,latu,lonin,latin),&
!$OMP& SHARED(idx_u,W_u), FIRSTPRIVATE(nx,ny) SCHEDULE(DYNAMIC)
      do j = 1, ny
      do i = 1, nx-1
        do k = 1, xin
        do l = 1, yin
          dis_array(k,l) = sqrt((lonu(i,j)-lonin(k))**2+(latu(i,j)-latin(l))**2)
        end do
        end do
        ...
        if (x.gt.1 .and. x.lt.xin .and. y.gt.1 .and. y.lt.yin) then
          ...
          do l = y-1,y
            ...
            if (score) then
              ...
              idx_u(i,j,3,:) = (/k+1,l+1/)
              idx_u(i,j,4,:) = (/k+1,l/)
              call calc_w( (/lonin(k),lonin(k+1)/),(/latin(l),latin(l+1)/), point, W_u(i,j,:) )
...
!___V___
    ...
    if (ex1 .and. ex2) then
      ...
      idx_v = 0
      W_v = 0
!$OMP PARALLEL DO DEFAULT(PRIVATE),SHARED(lonv,latv,lonin,latin),&
!$OMP& SHARED(idx_v,W_v), FIRSTPRIVATE(nx,ny) SCHEDULE(DYNAMIC)
      do j = 1, ny-1
      do i = 1, nx
        do k = 1, xin
        do l = 1, yin
          dis_array(k,l) = sqrt((lonv(i,j)-lonin(k))**2+(latv(i,j)-latin(l))**2)
        end do
        end do
...
!              call calc_w( loninu(k:k+1,l:l+1), latinu(k:k+1,l:l+1), point, W_v(i,j,:) )
              call calc_w( (/lonin(k),lonin(k+1)/),(/latin(l),latin(l+1)/), point, W_v(i,j,:) )
            end if
          end do
          end do
        else
...
!    goto 1001
    ...
    maskin3d = .true.
    where(abs(tempin-fv).lt.1.0) maskin3d = .false.
    CALL extrap3d(tempin,maskin3d,xin,yin,zin,5000,0)
    CALL extrap3d(saltin,maskin3d,xin,yin,zin,5000,0)
    ...
    maskin3d = .true.
    where(abs(uvelin-fv).lt.1.0) maskin3d = .false.
...
!$OMP& FIRSTPRIVATE(nx,ny) SCHEDULE(DYNAMIC)
    do k = 1,zin
      ...
      CALL extrap(tmp(1:nx-1,:,k),maskout(1:nx-1,:),nx-1,ny,500,0)
      write(*,*) 'uvel k=',k
    end do
!$OMP END PARALLEL DO
...
!$OMP& FIRSTPRIVATE(nx,ny) SCHEDULE(DYNAMIC)
    do k = 1,zin
      ...
      CALL extrap(tmp(:,1:ny-1,k),maskout(:,1:ny-1),nx,ny-1,500,0)
      write(*,*) 'vvel k=',k
    end do
!$OMP END PARALLEL DO
