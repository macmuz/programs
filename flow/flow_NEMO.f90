PROGRAM flow_NEMO
    ...
    real(kind=8), allocatable :: thetao(:,:,:),so(:,:,:)
    real(kind=8) :: time,tref,Cp,uflow,vflow,uflow_e,uflow_w,vflow_n,vflow_s
    real(kind=8) :: vinsalt,voutsalt,den,den1
...
!    end do
    ...
    cnt = 0
    CALL check(nf90_create( "NEMO_flow_1993.nc", NF90_NETCDF4, oncid ), 200)
...
!    CALL check(nf90_put_att( oncid, vid(3), 'units', 'm3/s'),223)
    ...
    do
        ...
        CALL check(nf90_get_var(ncid,varid,thetao),28)
        CALL check(nf90_inq_varid(ncid,"so",varid),27)
        CALL check(nf90_get_var(ncid,varid,so),28)
