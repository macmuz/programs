PROGRAM flow_NEMO
    ...
    real(kind=8) :: time,tref,Cp,uflow,vflow,uflow_e,uflow_w,vflow_n,vflow_s,vinflowb
...
!    date1 = (/1999,1,6/)
    ...
    CALL check(nf90_get_var(ncid,varid,uo),28)
...
!    end do
    ...
    cnt = 0
    CALL check(nf90_create( "NEMO_flow_test.nc", NF90_NETCDF4, oncid ), 200)
...
!    CALL check(nf90_put_att( oncid, vid(3), 'units', 'm3/s'),223)
    ...
    CALL check(nf90_put_att( oncid, vid(8), 'standard_name', 'v-section bottom water&
        & mass in south dir'),222)
    ...
    do
        ...
        CALL check(nf90_get_var(ncid,varid,vo),28)
