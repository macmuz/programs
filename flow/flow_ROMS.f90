PROGRAM flow_ROMS
    ...
    integer :: ncid,dimid,varid,nx,ny,nz,nt,vid(9)
    integer :: xd,yd,zd,td,cnt,myvarid(6)
    ...
    real(kind=8), allocatable :: temp(:,:,:,:),salt(:,:,:,:),tslice(:,:),sslice(:,:)
    real(kind=8), allocatable :: lon_u(:,:),lat_u(:,:),lon_v(:,:),lat_v(:,:)
    real(kind=8), allocatable :: dx(:,:),dy(:,:),time(:)
    real(kind=8) :: hc,tref,Cp,uflow_e,uflow_w,vflow_n,vflow_s,div,myh
    real(kind=8) :: top,bottom,mid,dz,vinsalt,voutsalt,den,den1
...
!    pref = '/users/work/mmuzyka/CSDIR/metro_560x600_uerraGLS2/run/baltic'
    pref = '/users/work/mmuzyka/CSDIR/metro_560x600_uerraGLSold/run/baltic'
...
!    date1 = (/1999,1,6/)
    ...
    allocate( temp(nx,ny,nz,nt),salt(nx,ny,nz,nt) )
    allocate( tslice(1+vi(2)-vi(1),nz),sslice(1+vi(2)-vi(1),nz) )
    ...
    cnt = 0
    CALL check(nf90_create( "ROMS_flow_uerraGLSold.nc", NF90_NETCDF4, oncid ), 200)
...
!    CALL check(nf90_put_att( oncid, vid(3), 'units', 'GJ'),223)
    ...
    do
        ...
        CALL check(nf90_inq_varid(ncid,"temp",myvarid(5)),129)
        CALL check(nf90_inq_varid(ncid,"salt",myvarid(6)),129)
        ...
        CALL check(nf90_get_var(ncid,myvarid(5),temp),134)
        CALL check(nf90_get_var(ncid,myvarid(6),salt),134)
        ...
        do t = 1,nt
          ...
          do k = 1,nz
            ...
            where(mask.eq.0) temp(:,:,k,t)=0.0
            where(mask.eq.0) salt(:,:,k,t)=0.0
          end do
        end do
        ...
        tslice = div*sum(temp(vi(1):vi(2),vj,:,:),3)
        sslice = div*sum(salt(vi(1):vi(2),vj,:,:),3)
...
!          heat = (temp-tref)*Cp*den*v          
        ...
        do i = 1,1+vi(2)-vi(1)
          if (mask(vi(1)+i-1,vj)) then
            myh = h(vi(1)+i-1,vj)
            top = -1*(vzeta(i)+(vzeta(i)+myh)*(hc*s_w(1)+myh*Cs_w(1))/(hc+myh))
            do k = 1,nz
              bottom = top
              top = -1*(vzeta(i)+(vzeta(i)+myh)*(hc*s_w(k+1)+myh*Cs_w(k+1))/(hc+myh))
              dz = abs(top-bottom)
              mid = (vzeta(i)+(vzeta(i)+myh)*(hc*s_r(k)+myh*Cs_r(k))/(hc+myh))
              CALL calc_den(tslice(i,k),sslice(i,k),mid,den1,den)
              if (vslice(i,k).gt.0) then
                vflow_n = vflow_n+vslice(i,k)*dx(vi(1)+i-1,vj)*dz
                voutsalt = voutsalt+vslice(i,k)*dx(vi(1)+i-1,vj)*dz*&
                    den*1e-6*sslice(i,k)
              else
                vflow_s = vflow_s+vslice(i,k)*dx(vi(1)+i-1,vj)*dz
                vinsalt = vinsalt+vslice(i,k)*dx(vi(1)+i-1,vj)*dz*&
                    den*1e-6*sslice(i,k)
              end if
            end do
          end if
...
!    CALL check( nf90_put_var( ncid, vid(7), d(1,1,:) ), 212 )
    ...
    deallocate( mask, u, v, zeta, temp, salt ) 
    deallocate( dx,dy,time, uslice, vslice, tslice, sslice )
